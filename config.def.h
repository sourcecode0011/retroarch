...
#if TLDEBUG
#define WERROR_A(FMT,...) __android_log_print(ANDROID_LOG_ERROR,"RetroArch", " (%s:%d) " FMT,__FILE__,__LINE__, ##__VA_ARGS__)

#define INTOFUNC() WERROR_A("into  function='%s'.",__FUNCTION__)
#define EXITFUNC() WERROR_A("exit function='%s' .",__FUNCTION__)
#else
#define WERROR_A(FMT,...) 
#define INTOFUNC()
#define EXITFUNC()
