...
static bool command_event_save_core_config(void)
{
   ...
   {
      WERROR_A("current core path:",RARCH_PATH_CORE);
      unsigned i;
...
bool command_event(enum event_command cmd, void *data)
{
   bool boolean              = false;
   INTOFUNC();
   WERROR_A("command_event cmd:%d",(int)cmd);
   switch (cmd)
   {
      ...
      case CMD_EVENT_AUDIO_STOP:
         WERROR_A("audio_driver_alive");
         //if (!audio_driver_alive())
         //   return false;
          WERROR_A("audio_driver_stop");
         //if (!audio_driver_stop())
          //  return false;
          WERROR_A("audio_driver_stop end");
         break;
