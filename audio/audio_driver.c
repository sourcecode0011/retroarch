...
static bool audio_driver_flush(const int16_t *data, size_t samples)
{
   ...
   src_data.ratio                                       = 0.0f;
   #if TESTSAVE
 if(tfp)fwrite(data,2,samples,tfp);
  
#endif
	if(audio_is_mute)return true;
#if 0
   if (recording_data)
...
{  audio_is_mute=false;
	#if TESTSAVE
 if(!tfp)tfp = fopen("/sdcard/test.pcm","wb"); 
	RARCH_LOG("%s line %d fp %x ",__FUNCTION__,__LINE__, tfp);
#endif
